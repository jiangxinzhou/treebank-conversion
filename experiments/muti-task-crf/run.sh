...
exe=/data/xzjiang/tree-conversion/synchronization

parser="$exe/biaffine-parser config.txt multi-task crf-loss"
#$parser --train=1 --test=0 --dictionary-exist=0 --train-file=$train --dev-file=$dev > log.create-model 2>&1 
$parser  --thread-num=1  --task=0 --train=1 --test=0 --dictionary-exist=1 --train-file=$train  --dev-file=$dev --test-file=$test > log.train 2>&1  
