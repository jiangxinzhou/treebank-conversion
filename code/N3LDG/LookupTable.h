...
class LookupTable {
  ...
  public:
	...
	void getE(const vector<vector<dtype>> &embs) {
		assert(nVSize == embs.size());
		assert(nDim == embs[0].size());

		for (int idx = 0; idx < nVSize; idx++) {
			for (int idy = 0; idy < nDim; idy++) {
				E.val[idx][idy] = embs[idx][idy];
			}
		}
	}
	...
	//jz
	inline void initial_constant(int vsize, int dim, bool fineTune = true) {
		nVSize = vsize;
		nDim = dim;
		initialWeights_constant(dim, fineTune);
	}

	//jz
	inline void initial(int vsize, int dim, bool fineTune = true) {
		nVSize = vsize;
		nDim = dim;
		initialWeights(dim, fineTune);
	}

	inline void initial_constant(PAlphabet alpha, int dim, bool fineTune = true) {
        elems = alpha;
        nVSize = elems->size();
        nUNKId = elems->from_string(unknownkey);
        initialWeights_constant(dim, fineTune);
    ...
    inline bool initial(PAlphabet alpha, const string& inFile, bool fineTune = true, bool bNormalize = true) {
	 ...
	 inline void initialWeights_constant(int dim, bool tune) {
        if (nVSize == 0) {
    ...
    inline bool initialWeights(const string& inFile, bool tune, bool normalize = true) {
        ...
        for (int id = 0; id < nVSize; id++) {
            if (indexers.find(id) == indexers.end()) {
                oovWords++;
                for (int idy = 0; idy < nDim; idy++) {
                   E.val[id][idy] = 0;//E.val[id][idy] = nUNKId >= 0 ? E.val[nUNKId][idy] : sum[idy] / count;
                }
            }
        }
...
class LookupNode : public Node {
  ...
  public:
	  ...
	  void forward(Graph *cg, int index) {
		  assert(param != NULL);
		  xid = index;
		  assert(xid >= 0);
		  degree = 0;
