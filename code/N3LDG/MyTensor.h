...
//useful functions
inline dtype fsigmoid_new(const dtype& x) {
	return (tanh(x) + 1) / 2;
}

inline dtype fminus_sigmoid_new(const dtype& x) {
	return 1 - (tanh(x) + 1) / 2;
}

inline dtype dsigmoid_new(const dtype& x, const dtype& y) {
	return 2.* y * (1.0 - y);
}

inline dtype dminus_sigmoid_new(const dtype& x, const dtype& y) {
	return -2. * y * (1.0 - y);
}
